...
The September 10 branding pass replaces the rail monogram with an original geometric KM, shared through `public/registry.svg` and repeated in the square favicon. Hover and keyboard focus add a cyan glow; reduced motion removes its transition. The home search is removed; Inventory retains search. The collection introduction uses the full user-supplied quote in a semantic blockquote, at 32px desktop and 20px mobile, without word-splitting animation.
