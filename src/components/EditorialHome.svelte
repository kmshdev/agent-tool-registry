...
<div class="editorial-home">
  <section class="hero"><div class="eyebrow"><WelcomeText /></div><h1><span use:revealText>Your next idea.</span><br/><span use:revealText>Already</span> <em use:revealText>equipped.</em></h1><div class="hero-bottom"><p>Tools, agents, and discoveries.<br/>One place to build what’s next.</p><form onsubmit={e=>{e.preventDefault();onsearch(query);}}><WorkIcon name="all"/><input aria-label="Find a capability" placeholder="Find your next tool" bind:value={query}/><button aria-label="Search registry"><MorphIcon icon="arrow"/></button></form></div></section>
  <div class="editorial-grid">{#each groups as group,i}<section class="feature" data-proximity-card><div class="section-label">/ {group.label}<span>{#if loading||data.revision<0}—{:else}<NumberFlow value={data.entries.filter(e=>e.source===group.id).length}/>{/if}</span></div><button class="feature-art art-{i}" onpointerenter={()=>hovered=i} onpointerleave={()=>hovered=-1} onclick={()=>{cue('press');onsource(group.id);}}><PatternStudy variant={i} caption={false}/><span class="art-index"><span class="figure-id">FIG. 0{i+1} / </span>Browse {group.label}</span><span class="art-action"><MorphIcon icon={hovered===i?'arrow':'plus'}/></span></button><h2 use:revealText>{group.title}</h2><p>{group.detail}</p><button class="pill-button" onclick={()=>onsource(group.id)}>Explore {group.label.toLowerCase()}<MorphIcon icon="arrow" size={16}/></button></section>{/each}</div>
