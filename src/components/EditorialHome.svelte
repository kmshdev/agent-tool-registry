<script lang="ts">
  ...
  let {data,connected,loading,onselect,onsource,onsearch,publicShowcase=false}:{data:Snapshot;publicShowcase?:boolean;connected:boolean;loading:boolean;onselect:(id:string)=>void;onsource:(source:string)=>void;onsearch:(query:string)=>void}=$props();
  let query=$state('');
...
<div class="editorial-home">
  <section class="hero terminal-hero"><div class="eyebrow"><span class="terminal-prompt">~/kmsh.dev</span> <span>{publicShowcase?'— notes from the internet':'— a personal workspace'}</span></div><h1><span use:revealText>Welcome to a</span><br/><span use:revealText>small corner of mind</span><br/><em use:revealText>KMSH.DEV</em><span class="terminal-caret" aria-hidden="true">▌</span></h1><div class="hero-bottom"><p>Hope you visit this page<br/>as seldomly as I do.</p><form onsubmit={e=>{e.preventDefault();onsearch(query);}}><WorkIcon name="all"/><input aria-label="Find a capability" placeholder={publicShowcase?"Find a discovery":"Find your next tool"} bind:value={query}/><button aria-label="Search registry"><MorphIcon icon="arrow"/></button></form></div></section>
  <div class="editorial-grid">{#each featuredProjects as group,i}<section class="feature" data-proximity-card><div class="section-label">/ Selected project<span>{String(i+1).padStart(2,'0')}</span></div><button class="feature-art art-{i}" onpointerenter={()=>hovered=i} onpointerleave={()=>hovered=-1} onclick={()=>{cue('press');onselect(group.id);}}>{#if i%2===0}<BoxStudy variant={i}/>{:else}<PatternStudy variant={i} caption={false}/>{/if}<span class="art-index"><span class="figure-id">FIG. 0{i+1} / </span>{group.language} / Open source</span><span class="art-action"><MorphIcon icon={hovered===i?'arrow':'plus'}/></span></button><h2 use:revealText>{group.name}</h2><p>{group.description}</p><button class="pill-button" onclick={()=>onselect(group.id)}>Explore project<MorphIcon icon="arrow" size={16}/></button></section>{/each}</div>
